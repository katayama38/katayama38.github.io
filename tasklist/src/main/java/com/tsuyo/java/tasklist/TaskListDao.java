...
public class TaskListDao {
    ...
    public List<HomeRestController.TaskItem> findAll() {
        String query = "SELECT * FROM tasklist";

        List<Map<String, Object>> result = jdbcTemplate.queryForList(query);
        List<HomeRestController.TaskItem> taskItems = result.stream()
                .map((Map<String, Object> row) -> new HomeRestController.TaskItem(
                        row.get("id").toString(),
